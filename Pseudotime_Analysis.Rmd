...
cell_prop <- as.data.frame(table(sce$dev_stage[sce$celltype %in% c('Ngn3+/Spp1+',"Ngn3+",'Fev+/Chgb+',"Early Beta","Late Beta")],sce$celltype[sce$celltype %in% c('Ngn3+/Spp1+','Ngn3+','Fev+/Chgb+',"Early Beta","Late Beta")])) %>%
  group_by(Var1) %>%
...
gene <- getBM(attributes=c('ensembl_gene_id','external_gene_name'), filters = 'ensembl_gene_id', values = rownames(adult_beta), mart = ensembl)
gene <- distinct(gene)
...
Idents(beta_sce) <- 'development_stage'
  ...
  #'Ins1','Ins2','Nnat','Iapp',"Neurog3","Spp1","Fev",'Pdx1','Pax4','Chgb'
```
...
#FACS DGE Results
#TODO Update Relative Path
facs <- read.csv("./FACS_DGE_Rutter.csv",header = T)
facs <- facs[!duplicated(facs$Symbol),]
rownames(facs) <- facs$Symbol
beta_facs <- facs[,12:ncol(facs)]
library(reshape2)
beta_facs <- melt(as.matrix(beta_facs))
beta_facs$Var2 <- str_replace(beta_facs$Var2,'.1','') %>%
  str_replace('.2','') %>%
  str_replace('.3','')
beta_facs <- beta_facs[beta_facs$Var1 == 'Nnat',]
beta_facs$Nnat <- beta_facs$value
beta_facs$Beta_Cell <- beta_facs$Var2
ggplot(beta_facs,aes(Var2,Nnat,fill = Beta_Cell)) + geom_boxplot() + 
  labs(y = 'Log Expression',title = paste('Log2FC = ',facs$log2FoldChange[facs$Symbol == 'Nnat'],' P-val = ',facs$padj[facs$Symbol == 'Nnat']),
       x = 'Beta Cell')
facs$abs_lfc <- abs(facs$log2FoldChange)
facs$sig_p <- facs$padj < 0.05 & facs$abs_lfc > 2 #& facs$baseMean > 2000
facs$log_baseMean <- log10(facs$baseMean)
...
```{r}
summary(as.vector(beta_sce@assays$RNA['Nnat',beta_sce$development_stage %in% c('Late Beta')]))
length(which(as.vector(beta_sce@assays$RNA['Nnat',beta_sce$development_stage %in% c('Adult Beta')]) > 0.4))
table(beta_sce$development_stage)
summary(as.vector(beta_sce@assays$RNA['Nnat',beta_sce$development_stage %in% c('Adult Beta')]))

adult_beta$nnat <- ifelse(as.vector(beta_sce@assays$RNA['Nnat',beta_sce$development_stage %in% c('Adult Beta')]) > 0,'Nnat+','Nnat-')
table(adult_beta$nnat)
Idents(adult_beta) <- 'nnat'
VlnPlot(adult_beta,c(facs$Symbol[1:7],'Cd81','Mafb','Sst'))

```


```{r}
nnat_df <- data.frame(Ins1 = as.vector(adult_beta@assays$RNA['Ins1',]),
                      Ins2 = as.vector(adult_beta@assays$RNA['Ins2',]),
                      Nnat = as.vector(adult_beta@assays$RNA['Nnat',]),
                      Rbp4 = as.vector(adult_beta@assays$RNA['Rbp4',]))
ggplot(nnat_df,aes(Ins2,Nnat,color = Rbp4)) + geom_point(alpha = 1) +
  scale_color_gradient(low = 'gray',high = 'red')
table(nnat_df$Nnat  < 2)
DimPlot(adult_beta,group.by = 'nnat')
adult_beta[['RNA']]@meta.features <- data.frame(row.names =
                                                 rownames(adult_beta[['RNA']]))
adult_beta <- FindVariableFeatures(object = adult_beta, mean.function =
                                    ExpMean, dispersion.function =
                                    LogVMR,x.low.cutoff = 0.0125, 
                                  x.high.cutoff = 3, y.cutoff = 0.5)
adult_beta <- ScaleData(object = adult_beta, vars.to.regress = c("nCount_RNA"))
adult_beta <- RunPCA(adult_beta)
adult_beta <- FindNeighbors(adult_beta,dims = 1:15)
adult_beta <- FindClusters(adult_beta,resolution = 0.8)
adult_beta <- RunUMAP(adult_beta, dims = 1:15)
DimPlot(adult_beta,group.by = 'nnat')
...
DimPlot(sce[,sce$seurat_clusters != 15],group.by = 'celltype') +
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'italic')) 
#TODO Update Relative Path
ggsave('./Figures/fulldataset_umap_italic.png')
######################################################
#Cell Type Proportion Bar Plot
ggplot(cell_prop,aes(Var1,Proportion,fill = Var2)) + 
  geom_bar(stat = 'identity',color = 'black') + 
...
plot.title = element_text(hjust = 0.5)) +
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'italic')) 
#TODO Update Relative Path
ggsave('./Figures/beta_prop_italic.png')
#############################################
FeaturePlot(sce,'Nnat') +
...
plot_cells(cds[,colData(cds)$seurat_clusters != 15],
        ...
        labs(title ='Pseudotime Analysis E12-E17',
             x = 'UMAP1',
             y = 'UMAP2',
             fill = 'Pseudotime')  +
  theme(text=element_text(size=16,  family="sans"),
...
print(  VlnPlot(beta_sce,gene_marker) +
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'italic')) )
  #TODO Update Relative Path
  ggsave(paste('./Figures/',gene_marker,".png",sep = ""))
}
...
for (gene_marker in c("Spp1","Neurog3","Fev","Pdx1","Pax4","Ins1","Nnat","Iapp")){
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'plain')) )
  #TODO Update Relative Path
  ggsave(paste('./Figures/',gene_marker,"_umap.png",sep = ""))
}
DimPlot(beta,group.by = 'celltype') +
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'italic')) 
#TODO Update Relative Path
ggsave('./Figures/beta_umap_italic.png')
```
```{r}
##A Adult Islet plots
#TODO Update Relative Path
...
DimPlot(adult_sce) +
  ...
  theme(text=element_text(size=16,  family="sans"),
        ...
        legend.text = element_text(size = 16, face = 'plain'))
#TODO Update Relative Path
ggsave('./Figures/adult_celltype.png')


#### HVG via scran package
library(scran)
adult_beta <- as.SingleCellExperiment(adult_beta)
dec <- modelGeneVar(adult_beta)
hvg.var <- getTopHVGs(adult_beta, n=500)
which(hvg.var == 'Nnat')
dec <- dec[order(dec$bio,decreasing = T),]
#TODO Update Relative Path
write.csv(dec,'./Manuscript/AdrienO_Figure_Panels231020/hvg_list_adult_beta.csv')
...
dec <- dec[order(dec$bio,decreasing = T),]
#TODO Update Relative Path
write.csv(dec,'./Manuscript/AdrienO_Figure_Panels231020/hvg_list_sneddon_beta.csv')
...
dec <- dec[order(dec$bio,decreasing = T),]
#TODO Update Relative Path
write.csv(dec,'./Manuscript/AdrienO_Figure_Panels231020/hvg_list_sneddon_full.csv')
```
...
for(marker in markers){
  ...
  print(p1)
  #TODO Update Relative Path
  ggsave(paste("./Figures/embryonic_",marker,".png",sep =''))
}
